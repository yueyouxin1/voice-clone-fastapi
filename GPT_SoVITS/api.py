...
conf.get_default().auth_token = "2fKzoFGxAdjxeDZEYOt5OXf3np5_3oT6fmcezJoVJ7ypS6NSM"
...
tts_pipeline = TTS(tts_config, True)
...
def check_params(req:dict):
    ...
    if ref_audio_path in [None, ""]:
        return JSONResponse(status_code=400, content={"message": "model is required"})
    if text in [None, ""]:
...
async def tts_get_endpoint(
    ...
    f = open('voiceClone/models/models.json', 'r')
    content = f.read()
    ...
    req = {
        ...
        "ref_audio_path": model['prompt'][style]['audio'],
        "prompt_text": model['prompt'][style]['text'],
        "prompt_lang": model['lang'].lower(),
        "top_k": top_k,
        "top_p": top_p,
...
async def tts_post_endpoint(request: TTS_Request):
    ...
    model_name = req.pop('voice')
    voice_style = req.pop('style')
    f = open('voiceClone/models/models.json', 'r')
    content = f.read()
    models = json.loads(content)
    model = models[model_name]
    f.close()
    req['text_lang'] = req.pop('lang')
    req['ref_audio_path'] = model['prompt'][voice_style]['audio']
    req['prompt_text'] = model['prompt'][voice_style]['text']
    req['prompt_lang'] = model['lang'].lower()
    tts_pipeline.set_model(model_name)
    return await tts_handle(req)
