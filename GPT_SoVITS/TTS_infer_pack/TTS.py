...
class TTS_Config:
    default_configs={
                ...
                "cnhuhbert_base_path": "GPT_SoVITS/pretrained_models/chinese-hubert-base",
                "bert_base_path": "GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
                "models_config_path": "voiceClone/models/models.json"
            }
    configs:dict = None
    def __init__(self, configs: Union[dict, str]=None):
        ...
        self.models_config_path = self.configs.get("models_config_path", None)
        ...
        if (self.models_config_path in [None, ""]) or (not os.path.exists(self.models_config_path)):
            self.models_config_path = self.default_configs['models_config_path']
            print(f"fall back to default models_config_path: {self.models_config_path}")            
        self.update_configs()
...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config], use_preload: bool = False):
    ...
    def set_model(self, model_name):
        ...
        self.vits_model = self.loaded_models[model_name]['vits']
        self.t2s_model = self.loaded_models[model_name]['gpt'] 
        
    def preload_models(self,):
        f = open(self.configs.models_config_path, 'r')
        content = f.read()
        ...
        for model_name, model_info in models_info.items():
            ...
            # 这里调用 init_vits_weights 方法加载模型
            # 假设 tts_pipeline 是包含 init_vits_weights 方法的实例
            vits_instance = self.init_vits_weights(model_info['vits'])
            gpt_instance = self.init_t2s_weights(model_info['gpt'])
            # 存储预加载的模型实例
